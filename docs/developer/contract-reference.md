...
For user-facing explanations, see [Contract Functions](../contract/functions.md) and [Data Model](../contract/data-model.md).
...
Published via the `#[contractevent]` macro, each indexed by `payment_id` as a topic so they can be filtered per-payment:
